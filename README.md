...
- **V2** — **knock (M9) and search (M10) are built.** **DMs (M11) are started
  and not finished**: you can start one from somebody's card and hold a
  conversation nobody else's client is told about, but a DM still leaks through
  search, the media grid and the export until the last task lands, so don't run
  a server where people use them yet. Voice rooms and ambient voice are planned and not started; V1 still has
